...
- 已完赛 8 场预置：小X账号的帖子（每场 2~3 条）、评论、预测记录，预测结算结果 +0/+1/+3 混合，用于个人中心展示。

### 4.3 运行时内容生成（混合方案）

虚拟时间推进使某场比赛变为 finished 时，后端除结算外**自动生成 1 条帖子 + 2 条评论**，作者由 4 个球迷账号轮换（如绿茵漫步者、看台第12人…），内容取自模板池（如"可惜最后被绝杀""预测果然应验"）。生成的作者均为真实 users 行，评审若要登录也不穿帮。
