...
| GET | /api/leaderboard | 公开（登录时附我的排名） | 积分排行榜：仅统计有预测记录的用户；按积分↓、猜中比分次数↓、id↑ 排序，同分同名次；含排名/昵称/积分/预测总数/已出结果/猜中比分/猜中胜负 |
| GET | /api/users/:id/predictions | 公开 | 用户预测主页：用户信息 + 全部预测记录（对阵/轮次/时间/状态/实际比分/预测/结算得分），按状态前端分为"过往预测（已出结果）"与"未来预测"；含统计（总/已出结果/猜中比分/猜中胜负/未中/累计得分） |
